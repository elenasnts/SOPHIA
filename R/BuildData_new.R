...
#' @examples
#' # Not yet
buildData_new <- function(cdm_bbdd,
                      cdm_schema,
                      results_sc,
                      cohortTable,
                      acohortId = 1){
  ...
  cer_vars <- FeatureExtraction::createAnalysisDetails(
    analysisId = 127,
    sqlFileName = "DomainConcept.sql",
    parameters = list(analysisId = 116,
                      analysisName = "cer",
                      ...
                      domainStartDate = "condition_start_date",
                      domainEndDate = "condition_start_date"),
    includedCovariateConceptIds = c(381591, 40394172, 4121341,
                                    45773166, 45950463, 45473560,
                                    45524966, 40323983, 3569809,
                                    45473560, 40323976, 4108361
),
    addDescendantsToInclude = FALSE)
...
transformToFlat <- function(covariateData){
  ...
  bbdd_covar <-  dplyr::mutate(
    ...
    variable = dplyr::if_else(stringr::str_sub(.data$covariateId, start = -3L) == 127, 'cer', .data$variable),
  ...
  bbdd_covar <- dplyr::mutate(
    .data = bbdd_covar,
    dplyr::across(dplyr::any_of(c('sex_female', 'sex_male', 'T2DM', 'obesity', 'angor', 'tia', 'cer', 'stroke',
...
buildFollowUp <- function(cdm_bbdd,
  ...
  cohort_event <- cohort[cohort$COHORT_DEFINITION_ID %in% c(acohortId, 3:7),]
  ...
  cohort_event$event[cohort_event$COHORT_DEFINITION_ID == 6] <- 'cer'
  ...
  bbdd_covar$i.ep_cer <- 0
  bbdd_covar$i.ep_cer[bbdd_covar$dintro < bbdd_covar$ep_cer &
                        bbdd_covar$ep_cer <= bbdd_covar$OBSERVATION_PERIOD_END_DATE] <- 1
  bbdd_covar$t.ep_cer <- as.numeric(pmin(bbdd_covar$ep_cer, bbdd_covar$OBSERVATION_PERIOD_END_DATE, na.rm = T) - bbdd_covar$dintro)
